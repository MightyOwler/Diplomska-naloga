...
ki sta ključni orodji pri obravnavi zakonov. Podrobno sta opisani v magistrskem delu \cite{Schneider_2016}. Začeli bomo z dokazom komutatorske leme, za katerega bomo potrebovali nekaj definicij.
...
\begin{definicija}
    ...
    \end{equation*}  
    imenujemo \emph{izginjajoča množica besede $w$ v grupi $G$}. Tu $w(g_1, \ldots, g_{k})$ označuje evalvacijo besede $w$ z elementi $g_1, \ldots, g_n$ v skladu z notacijo na začetku razdelka \ref{sec_osnovne_lastnosti_zakonov}.  
    \end{definicija}
...
w(g,h) = [w_1(g,h), w_2(g,h)] = [1_{G}, w_2(g,h)] = 1_{G}.
\end{equation*}  
...
Komutatorska lema podaja konstrukcijo, ki preprečuje pojav takšnih zapletov. V sledeči obliki se pojavi v magistrskem delu \cite{Schneider_2016}, po katerem so prirejeni dokazi.
...
\end{lema}
    \begin{dokaz}
        ...
        To lahko zaradi pogoja $k \ge 2$ vedno storimo. Netrivialni komutator je v splošnem aperiodična beseda, kot je prvi opazil Schützenberger v članku \cite{Schutzenberger_1959} med obravnavo enačb v prostih grupah.  Zato je beseda $[s, w_1]$ aperiodična dolžine največ $2(l(w_1)  + 1)$.
        ...
        \end{equation*}  
        in za vsako grupo $G$ veljata vsebovanosti\begin{equation*}
        ...
        Zdaj moramo le še ugotoviti, kako lahko besedi $v_1$ ter $v_2$ ustrezno združimo. Po lemi \ref{lem_posledica_nielsen_schreier} vemo, da je komutator $[v_1, v_2]$ trivialen natanko v primeru $v_1 = v_2^{\pm 1}$, ker sta $v_1$ in $v_2$ po predpostavki aperiodični. V tem primeru imamo \begin{equation*}
        \operatorname{Z}(G, w_1) = \operatorname{Z}(G, w_2)
        \end{equation*}  
         in lahko nastavimo $w := v_1$ ali $w := v_2$. Pri tem je zahtevani pogoj na dolžino besede $w$ očitno izpolnjen. Če imamo $v_1 \neq v_2^{\pm 1}$, nastavimo $w := [v_1, v_2] \neq 1_{F_k}$. Netrivialni komutatorji so aperiodične besede po \cite{Schutzenberger_1959}. Indukcijska predpostavka nam zagotavlja 
         \begin{equation*}
         l(w) \le 2m  \left(\frac{m}{2} + \sum_{i=1}^{m / 2} l(w_{i}) \right) + 2m \left(\frac{m}{2} + \sum_{i= m / 2 + 1}^{m} l(w_{i}) \right) = 2m \left( m + \sum_{i = 1}^{m} l(w_{i}) \right). \qedhere
         \end{equation*}
    \end{dokaz}
...
\begin{dokaz}
    ...
    \sum_{i = 1}^{m} l(w_{i}) \le m \max_{i = 1, \ldots, m} l(w_i). \qedhere
    \end{equation*}  
\end{dokaz}
...
ki je zakon v grupi $G$, saj velja \begin{equation*}
    Z(G, w) \supseteq \bigcup_{i = 1}^m Z(G, a^{n_i}) =  \bigcup_{i = 1}^m H_{n_i} \times G = G \times G. \qedhere
\end{equation*}\end{primer}
Čeprav je ta primer razmeroma preprost, je ključen pri praktično vseh konstrukcijah zakonov, kar bomo videli recimo na koncu razdelka \ref{sec_grupe_psl2q} pri obravnavi družine grup $\operatorname{PSL}_2(q)$. 
...
Naj bodo $A, B, C$ grupe in naj $\mathbf{1}$ označuje trivialno grupo. \emph{Kratko eksaktno zaporedje} je zaporedje homomorfizmov
\begin{equation*}
\mathbf{1} \to A \xrightarrow{\varphi} B \xrightarrow{\psi} C \to \mathbf{1},
\end{equation*}  
kjer je $\varphi$ je injektivni in $\psi$ surjektivni homomorfizem ter velja $\ker \psi = \operatorname{im} \varphi$.
\end{definicija}

Razširitvena lema je v podobni obliki podana v \cite[str.~9--10]{Schneider_2016}.
...
\begin{lema}\label{lem_razsiritvena_lema}
    ...
    Potem obstaja netrivialni $k$-črkovni zakon v grupi $G$ dolžine največ $l(w_N) l( w_{ G / N } )$.
\end{lema}
\begin{dokaz}
    Dokaz je prirejen po \cite[str.~10]{Schneider_2016} in obravnava dva možna primera oblike zakona $w_{ G / N }$. 
     V prvem primeru je $w_{ G / N }$ enočrkovni zakon, torej je oblike $w_{G / N} = s^{n}$ za neko črko $s \in S$ in neničelno celo število $n$. V tem primeru je za vsak $i = 1, \ldots, k$ beseda $a_i^n$ zakon v kvocientu $G / N$. Definirajmo besedo \begin{equation*}
        ...
        je beseda $w$ netrivialni zakon v grupi $G$. Enak razmislek izvedemo v primeru, ko je $w_{ G / N }$ oblike $(s^{n})^u$ za neko besedo $u \in F_k$, saj ga s konjugiranjem prevedemo v enočrkovno obliko.
...
\begin{primer}\label{prm_razsiritvena}
    S pomočjo razširitvene leme \ref{lem_razsiritvena_lema} lahko dokažemo, da za vsako naravno število $n$ obstaja zakon $w \in F_2$ dolžine $8$ v diedrski grupi $D_{2n} = \langle r, Z \rangle$. Ker ima podgrupa $\langle r \rangle \subseteq D_{2n}$ indeks 2, je edinka,
    zato lahko tvorimo kratko eksaktno zaporedje \begin{equation*}
        ...
        \end{equation*}
        Ker je podgrupa $\langle r \rangle$ Abelova, je v njej zakon beseda $[a, b] = aba^{-1} b^{-1}$, grupa $D_{2n} / \langle r \rangle$ pa je moči $2$ in je zato v njej zakon beseda $a^2$. Po razširitveni lemi \ref{lem_razsiritvena_lema} torej obstaja beseda $w \in F_2$ dolžine $l(w) \le  8$, ki je zakon v $D_{2n}$.
        Če sledimo konstrukciji izreka, vidimo, da je to natanko beseda $w = [a^2, b^2] = a^2 b^2 a^{-2} b^{-2}$.   
\end{primer}

Iz primera je razvidno, da je moč razširitvene leme še posebej izrazita, kadar ima grupa kakšno edinko z lepimi lastnosti, kot so na primer rešljivost, nilpotentnost ali celo Abelovost. Od tod sledi tudi, da so s stališča preučevanja zakonov \emph{virtualno nilpotentne} oziroma \emph{rešljive grupe} -- torej grupe, ki imajo nilpotentno oziroma rešljivo edinko končnega indeksa --
praktično enake nilpotentnim oziroma rešljivim grupam. Naravna posledica razširitvene leme je tudi dejstvo, da bistveno vlogo pri iskanju kratkih zakonov igrajo enostavne grupe, saj lahko problem iskanja zakonov v neenostavnih grupah vedno prevedemo na dva manjša; na problema edinke in njenega kvocienta.  