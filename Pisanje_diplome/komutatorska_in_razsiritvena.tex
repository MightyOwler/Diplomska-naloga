...
% TODO predstavi Schutzenbergovo lemo ... lahko skico dokaza + odstrani referenco na to lemo
...
\begin{dokaz}
    Dokaz je prirejen po \cite[str.~10]{Schneider_2016} in obravnava dva možna primera oblike zakona $w_{ G / N }$. 
     V prvem primeru je $w_{ G / N }$ enočrkovni zakon, torej je oblike $w_{G / N} = s^{n}$ za neko črko $s \in S$ in neničelno celo število $n$. V tem primeru je za vsak $i = 1, \ldots, k$ beseda $a_i^n$ zakon v kvocientu $G / N$. Definirajmo besedo \begin{equation*}
        ...
        Če $w_{G / N}$ ni enočrkovni zakon ali konjugat enočrkovnega zakona, lahko brez škode za splošnost predpostavimo, da se začne s črko $a_1$. Potem ga pretvorimo v obliko $w_{ G / N} = a_1 w_{ G / N}' a_2$, kjer se $w_{ G / N}'$ niti ne začne z $a_1^{-1}$ niti ne konča z $a_2^{-1}$.
